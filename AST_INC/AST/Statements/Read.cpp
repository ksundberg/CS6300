...
cs6300::Read::emit()
{
  ...
  for (auto &&val : values) {
    ...
    if (val->type() == BuiltInType::getInt()) {
      block->instructions.push_back(ThreeAddressInstruction(ThreeAddressInstruction::ReadInt, b->getLabel(), 0, 0));
    }
    if (val->type() == BuiltInType::getChar()) {
      block->instructions.push_back(ThreeAddressInstruction(ThreeAddressInstruction::ReadChar, b->getLabel(), 0, 0));
    }
