  ...
  namespace consts
  {
    ...
    const struct
    {
      ...
      const char* detail;
        ...
        {SIGILL, "SIGILL", "Illegal instruction", "Generally due to a "
...
namespace base
{
  ...
  namespace utils
  {
    ...
    template <typename T>
    static inline typename std::enable_if<std::is_pointer<T*>::value,
                                          void>::type
  ...
  namespace utils
  {
    ...
    public:
...
#if ELPP_OS_WINDOWS
        std::locale elppUnicodeLocaleWindows(
          elppUnicodeLocale, new std::codecvt_utf8_utf16<wchar_t>);
      ...
      static bool createPath(const std::string& path)
      {
        ...
        {
          ELPP_INTERNAL_ERROR(
            "Error while creating path [" << path << "]", true);
          return false;
        }
        return true;
      }
    ...
    public:
      ...
      static inline char* addToBuff(const char* str,
                                    char* buf,
                                    const char* bufLim)
      {
      ...
      {
        STRCPY(buff, "", lim);
        ELPP_UNUSED(lim); // For *nix we dont have anything using lim in above
                          // STRCPY macro
        return buff;
      }
...
#define ELPP_INITI_BASIC_DECLR                                                 \
  ...
  {                                                                            \
    ...
    public:
      ...
      friend inline base::type::ostream_t& operator<<(
        base::type::ostream_t& os, const RegistryWithPred& sr)
      {
...
namespace base
{
  ...
  public:
    ...
    void parseFromFormat(const base::type::string_t& userFormat)
    {
      ...
      conditionalAddFlag(
        base::consts::kAppNameFormatSpecifier, base::FormatFlags::AppName);
      conditionalAddFlag(
        base::consts::kSeverityLevelFormatSpecifier, base::FormatFlags::Level);
      conditionalAddFlag(base::consts::kSeverityLevelShortFormatSpecifier,
                         base::FormatFlags::LevelShort);
      conditionalAddFlag(
        base::consts::kLoggerIdFormatSpecifier, base::FormatFlags::LoggerId);
      conditionalAddFlag(
        base::consts::kThreadIdFormatSpecifier, base::FormatFlags::ThreadId);
      conditionalAddFlag(
        base::consts::kLogFileFormatSpecifier, base::FormatFlags::File);
      conditionalAddFlag(
        base::consts::kLogFileBaseFormatSpecifier, base::FormatFlags::FileBase);
      conditionalAddFlag(
        base::consts::kLogLineFormatSpecifier, base::FormatFlags::Line);
      conditionalAddFlag(
        base::consts::kLogLocationFormatSpecifier, base::FormatFlags::Location);
      conditionalAddFlag(
        base::consts::kLogFunctionFormatSpecifier, base::FormatFlags::Function);
      conditionalAddFlag(
        base::consts::kCurrentUserFormatSpecifier, base::FormatFlags::User);
      conditionalAddFlag(
        base::consts::kCurrentHostFormatSpecifier, base::FormatFlags::Host);
      conditionalAddFlag(
        base::consts::kMessageFormatSpecifier, base::FormatFlags::LogMessage);
...
public:
                  ...
                  const std::string& value)
  {
    ...
    {
      unsafeSetGlobally(
        configurationType, value, false); // Again this is not unsafe either
    }
  }
  ...
  void setRemainingToDefault(void)
  {
    ...
    unsafeSetIfNotExist(
      Level::Global, ConfigurationType::ToStandardOutput, std::string("true"));
    unsafeSetIfNotExist(
  ...
  public:
                              ...
                              Configurations* base = nullptr)
    {
      ...
      while (fileStream_.good())
      {
        ...
        ELPP_ASSERT(
          parsedSuccessfully, "Unable to parse configuration line: " << line);
      }
  ...
  private:
    ...
    unsigned long getULong(std::string confVal)
    {
      ...
      if (!valid)
      {
        valid = false;
        ELPP_ASSERT(
          valid, "Configuration value not a valid integer [" << confVal << "]");
        return 0;
      }
    ...
    void setModules(const char* modules)
    {
      ...
      auto insert = [&](std::stringstream& ss, base::type::VerboseLevel level)
      {
        if (!base::utils::hasFlag(
              LoggingFlag::DisableVModulesExtensions, *m_pFlags))
        {
...
#define ELPP el::base::elStorage
            ...
            ELPP_INTERNAL_ERROR(
              "Log file for ["
                ...
                << "has not been configured but [TO_FILE] is configured to "
                   "TRUE. [Logger ID: " << m_data->logMessage()->logger()->id()
                << "]",
              false);
          }
        }
  ...
  protected:
    void handle(const LogDispatchData* data)
    {
      ...
      // Save resources and only queue if we want to write to file otherwise
      // just ignore handler
...
namespace base
{
  ...
  public:
    ...
    void dispatch(void)
    {
      ...
      {
        tc->validateFileRolling(
          m_logMessage.level(), ELPP->preRollOutCallback());
      }
...
#undef ELPP_ITERATOR_CONTAINER_LOG_FIVE_ARG
    ...
    inline MessageBuilder& writeIterator(Iterator begin_,
                                         Iterator end_,
                                         std::size_t size_)
    {
      m_logger->stream() << ELPP_LITERAL("[");
  ...
  protected:
                          ...
                          bool needLock = true)
    {
      ...
      if (m_logger == nullptr)
      {
        ...
        Writer(Level::Debug, m_file, m_line, m_func).construct(
          1, base::consts::kDefaultLoggerId)
    ...
    void triggerDispatch(void)
    {
      ...
      {
        base::Writer(Level::Warning, m_file, m_line, m_func).construct(
          1, base::consts::kDefaultLoggerId)
...
namespace base
{
  ...
  private:
      ...
      struct timeval startTime) const
    {
      ...
      {
        base::type::stringstream_t ss;
        ss << base::utils::DateTime::getTimeDifference(
                m_endTime, startTime, m_timestampUnit) << " "
...
public:
  ...
  static inline Logger* reconfigureLogger(const std::string& identity,
                                          const Configurations& configurations)
  {
    return Loggers::reconfigureLogger(
      Loggers::getLogger(identity), configurations);
  }
...
#define ELPP_INIT_EASYLOGGINGPP(val)                                           \
  ELPP_INITI_BASIC_DECLR namespace el                                          \
  {                                                                            \
    namespace base                                                             \
    {                                                                          \
